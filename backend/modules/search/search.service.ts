...
export class SearchService {
  ...
  async getSuggestions(query: string, limit = 10): Promise<string[]> {
    try {
      ...
      const suggest = response.suggest.product_suggest[0] as any;
      return suggest.options.map((option: any) => option.text);
    } catch (error) {
  ...
  async getIndexStats(): Promise<any> {
    try {
      ...
      return {
        index: this.indexName,
        documentCount: response.indices[this.indexName].total.docs.count,
        indexSize: response.indices[this.indexName].total.store.size_in_bytes,
