...
export class OpenAIService {
  ...
  /**
   * Normalize product information using OpenAI
   */
  async normalizeProduct(request: NormalizationRequest): Promise<NormalizationResult> {
    try {
      ...
      this.logger.log(`Normalizing product with OpenAI: ${request.productName}`);
      ...
      const response = await this.client.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert product data normalization specialist. Return only valid JSON responses.',
          },
          {
            role: 'user',
            content: prompt,
      ...
      const result = this.parseNormalizationResponse(content);
      
      this.logger.log(`OpenAI normalization completed: ${result.normalizedName}`);
      return result;
    } catch (error) {
      this.logger.error('OpenAI normalization failed:', error);
      throw error;
    }
  }

  /**
   * Deduplicate products using OpenAI
   */
  async deduplicateProducts(request: DeduplicationRequest): Promise<DeduplicationResult> {
    try {
      ...
      this.logger.log(`Deduplicating products with OpenAI: ${request.products.length} products`);
      ...
      const response = await this.client.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert product deduplication specialist. Return only valid JSON responses.',
          },
          {
            role: 'user',
            content: prompt,
      ...
      const result = this.parseDeduplicationResponse(content);
      
      this.logger.log(`OpenAI deduplication completed: ${result.groups.length} groups found`);
      return result;
    } catch (error) {
      this.logger.error('OpenAI deduplication failed:', error);
      throw error;
    }
  }
...
Return only the JSON response, no additional text.
  ...
  /**
   * Build deduplication prompt for OpenAI
   */
...
Return only the JSON response, no additional text.
  ...
  /**
   * Parse normalization response from OpenAI
   */
  private parseNormalizationResponse(responseText: string): NormalizationResult {
    try {
      ...
      this.logger.error('Failed to parse OpenAI normalization response:', error);
      throw new Error('Invalid response format from OpenAI');
    }
  }

  /**
   * Parse deduplication response from OpenAI
   */
  private parseDeduplicationResponse(responseText: string): DeduplicationResult {
    try {
      ...
      this.logger.error('Failed to parse OpenAI deduplication response:', error);
      throw new Error('Invalid response format from OpenAI');
    }
  }
